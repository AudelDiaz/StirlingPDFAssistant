...
class BotHandlers:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        greeting = self._t(
            update, "start_greeting", name=update.effective_user.first_name
        )
    ...
    async def list_users_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        users_list = "\n".join(
            [
                f"- `{uid}`" + (" (Owner)" if uid == self.owner_id else "")
                for uid in users
            ]
        )
    ...
    async def set_limit_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not context.args:
            await update.message.reply_text(
                self._t(update, "admin_limit_usage"), parse_mode="Markdown"
            )
            return
        try:
    ...
    async def request_access_callback(
        ...
        keyboard = [
            [KeyboardButton(self._t(update, "req_access_button"), request_contact=True)]
        ]
        ...
        await context.bot.send_message(
            ...
            text=self._t(update, "req_access_prompt"),
    ...
    async def handle_contact(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    self._t(update, "req_approve"),
                    callback_data=f"appr_acc:{c.user_id}",
                ),
                InlineKeyboardButton(
                    self._t(update, "req_deny"), callback_data=f"deny_acc:{c.user_id}"
                ),
            ]
        ]
        await context.bot.send_message(
        ...
        await update.message.reply_text(
            self._t(update, "req_access_sent"), reply_markup=ReplyKeyboardRemove()
        )

    async def approve_access_callback(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        ...
        if self.user_manager.add_user(target_id):
            await query.edit_message_text(
                self._t(update, "req_approved_log", user_id=target_id)
            )
    ...
    async def deny_access_callback(
        ...
        await update.callback_query.edit_message_text(
            self._t(update, "req_denied_log", user_id=tid)
        )
    ...
    async def merge_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        context.chat_data["merge_queue"] = (
            []
        )  # List of (filename, file_content, mime_type)

        keyboard = [
            [
                InlineKeyboardButton(
                    self._t(update, "merge_btn_finish"), callback_data="merge_finish"
                ),
                InlineKeyboardButton(
                    self._t(update, "merge_btn_cancel"), callback_data="merge_cancel"
                ),
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def merge_finish_callback(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        query = update.callback_query
        ...
        await query.answer()
        await query.edit_message_text(
            text=self._t(update, "merge_processing", count=len(queue))
        )
            ...
            try:
                ...
                for i, (fname, content, mime) in enumerate(queue):
                    if len(queue) > 2:
                        await query.edit_message_text(
                            text=self._t(
                                update, "merge_step", current=i + 1, total=len(queue)
                            )
                        )
                ...
                merged_result = await self.stirling_client.execute(
                    self.merge_tool, file_contents=final_pdf_list
                )
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text=self._t(update, "err_merge", error=str(e)),
                )

    async def merge_cancel_callback(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        context.chat_data["merge_active"] = False
        context.chat_data["merge_queue"] = []
        await update.callback_query.answer()
        await update.callback_query.edit_message_text(
            self._t(update, "merge_cancelled")
        )
    ...
    async def handle_document(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.chat_data.get("merge_active"):
            ...
            await update.message.reply_text(
                self._t(update, "msg_added_queue", count=count)
            )
            return
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    self._t(update, "btn_compress"), callback_data="action_compress"
                ),
                InlineKeyboardButton(
                    self._t(update, "btn_ocr"), callback_data="action_ocr"
                ),
            ],
            [
                InlineKeyboardButton(
                    self._t(update, "btn_password"), callback_data="action_password"
                ),
                InlineKeyboardButton(
                    self._t(update, "btn_to_word"), callback_data="action_to_word"
                ),
            ],
            [
                InlineKeyboardButton(
                    self._t(update, "btn_scanner"), callback_data="action_scanner"
                ),
                InlineKeyboardButton(
                    self._t(update, "btn_split"), callback_data="action_split"
                ),
            ],
            [
                InlineKeyboardButton(
                    self._t(update, "btn_redact"), callback_data="action_redact"
                )
            ],
        ]
        await update.message.reply_text(
            self._t(update, "msg_received_file", name=doc.file_name),
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="Markdown",
        )
    ...
    async def handle_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.chat_data.get("merge_active"):
            ...
            count = len(context.chat_data["merge_queue"])
            await update.message.reply_text(
                self._t(update, "msg_img_added_queue", count=count)
            )
            return
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    self._t(update, "btn_img_to_pdf"), callback_data="action_img_to_pdf"
                )
            ]
        ]
        ...
        await update.message.reply_text(
            self._t(update, "msg_received_img"),
            reply_markup=InlineKeyboardMarkup(keyboard),
        )
    ...
    async def handle_callback_query(
        ...
        if action.startswith(("appr_acc", "deny_acc", "req_acc")):
            return
        if action == "merge_finish":
            return await self.merge_finish_callback(update, context)
        if action == "merge_cancel":
            return await self.merge_cancel_callback(update, context)
        ...
        async with self.semaphore:
            try:
                ...
                if action == "action_compress":
                    ...
                    out = (
                        fname.replace(".jpg", ".pdf")
                        .replace(".png", ".pdf")
                        .replace("converted", "document")
                    )
                    ...
                    await query.edit_message_text(
                        text=self._t(update, "action_scanning")
                    )
                    res = await self.stirling_client.execute(
                        self.scanner_tool, file_content=content, filename=fname
                    )
                ...
                await context.bot.send_document(
                    chat_id=update.effective_chat.id, document=res, filename=out
                )
            except Exception as e:
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text=self._t(update, "err_generic", error=str(e)),
                )
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if url_match and not any(
            ...
            status = await update.message.reply_text(
                self._t(update, "action_converting_url")
            )
            async with self.semaphore:
                try:
        ...
        if chat_data.get("awaiting_password"):
            await self._process_tool_with_input(
                update,
                context,
                self.password_tool,
                "awaiting_password",
